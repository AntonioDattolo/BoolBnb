...
@section('content')
	...
	<form action="{{ route('admin.suite.store') }}" method="POST" enctype="multipart/form-data">
		...
		<div class="w-50 m-5">
			...
			<input type="number" class="form-control" id="suite_room" placeholder="Number of rooms" name="room" min="1"
				max="20" value="{{ old('room') }}" required>
		</div>
		@error('room')
			<span class="bg-danger" role="alert">
				<strong>{{ $message }}</strong>
		...
		<div class="w-50 m-5">
			...
			<input type="number" class="form-control" id="suite_bed" placeholder="Number of beds" name="bed" min="1"
				max="20"value="{{ old('bed') }}" required>
		</div>
		@error('bed')
			<span class="bg-danger" role="alert">
				<strong>{{ $message }}</strong>
